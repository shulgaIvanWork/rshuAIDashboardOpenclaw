...
export const UF = {
  ...
  // PostSale / NPS
  NPS_SCORE:         'UF_CRM_1703146018908',     // Индекс NPS (для отчетов) — число 1-10
  NPS_CATEGORY:      'UF_CRM_1701751401798',     // Индекс NPS — список (1-10)
  NPS_COMMENT:       'UF_CRM_1701751424430',     // Комментарий NPS
  POSTSALE_SRC_ID:   'UF_CRM_1476128242',       // Сделка PostSale: ID исходной сделки Sale (заполняется БП №596)
  LEARNER_STATUS:    'UF_CRM_5DF2528C641D4',     // Статус участника обучения (список: Промоутер/Пассивный/Детрактор/Нет контактов)
...
export const CAT_POSTSALE = 9;   // Post Sale (NPS-опросы)
...
// ── PostSale — стадии ──────────────────────────────────────────────────────────
// Стадии, которые считаются «анкета отправлена»
export const POSTSALE_SENT_STAGES = new Set([
  'UC_EPPAPR',  // Запрошен NPS WA
  'UC_0Z9C0Q',  // Запрошен NPS письмом
  'UC_UJ3AGH',  // В работе
  'UC_9N3TCX',  // Заполнил NPS
  '1',          // Детрактор
  'WON',        // Успешно завершено
  'LOSE',       // Нет связи (отправляли, не ответил)
]);

// Стадии, которые считаются «NPS заполнен»
...
// ── Статус участника обучения (UF_CRM_5DF2528C641D4) ────────────────────────
export const LEARNER_STATUS = {
  PROMOTER:  '12588',  // Участник-Промоутер
  PASSIVE:   '12589',  // Участник-Пассивный (нейтрал)
  DETRACTOR: '12590',  // Участник-Детрактор
  NO_CONTACT:'21747',  // Участник-Нет контактов
};

// Все ID статусов, которые считаются «данные NPS получены» (исключая «Нет контактов»)
export const LEARNER_STATUS_FILLED = new Set([
  LEARNER_STATUS.PROMOTER,
  LEARNER_STATUS.PASSIVE,
  LEARNER_STATUS.DETRACTOR,
]);
