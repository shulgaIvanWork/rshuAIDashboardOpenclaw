...
def is_paid(r):
    """Оплата: UF_DATE_PAY_1C + WON + >0. Без fallback на CLOSEDATE."""
    if r["OPP"] < MIN_OPP:
        return False
    if r["SEM"] != "S":
        return False
...
m_oom_prev = metrics(ws_prev, is_oom_block=True)
m_oom_cur  = metrics(ws_cur,  is_oom_block=True)
...
for r in rows:
    if is_paid(r) and get_pay_year(r) == YEAR:
        cid = str(r.get("COMPANY_ID") or cc.get(r["ID"], {}).get("COMPANY_ID", "0"))
        if cid and cid != "0":
