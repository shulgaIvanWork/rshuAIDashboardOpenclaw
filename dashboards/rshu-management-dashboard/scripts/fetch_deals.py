...
SELECT = [
    ...
    "CLOSED", "ASSIGNED_BY_ID", "SOURCE_ID", "SOURCE_DESCRIPTION",
    "UTM_SOURCE", "UTM_MEDIUM", "UTM_CAMPAIGN", "UTM_TERM", "UTM_CONTENT",
    "COMPANY_ID", "CONTACT_ID", "LEAD_ID",
    "BEGINDATE",
    "UF_DATE_PAY_1C",
    "UF_FORMAT",                           # Формат обучения (КОМ, ООМ, СДО...)
    "UF_CRM_1498466811",                    # Направление обучения
    ...
    "UF_CRM_1474975772",                    # Согласованная дата оплаты
    "UF_CRM_1753341391806",                 # Дата отказа
    "UF_CRM_DATE_START_LEARN",              # Дата начала обучения
    "UF_CRM_DATE_END_LEARN",                # Дата окончания обучения
    "UF_CRM_1697096074",                    # Продукт
    "UF_CRM_1707848027366",                 # Товарная позиция (ID строки)
    "UF_CRM_63BF4EADDAF43",                 # Товары (массив ID)
    "UF_CRM_THEMES",                        # Тематика
    "UF_CRM_1448605527",                    # Канал регистрации
    "UF_CRM_5D133690E1",                    # roistat
    "UF_CRM_YA_CID",                        # Yandex ClientID
    "UF_CRM_YA_COUNTER_ID",                 # Yandex CounterID
    "UF_CRM_APP_PL_DEAL_REGION",            # Регион
    "UF_CRM_APP_PL_DEAL_LOCALITY",          # Город
    "UF_CRM_COURSE_NAME",                   # Название курса
    "UF_CRM_COURSE_URL",                    # URL курса
    "PROBABILITY",
]
PAGE       = 50
...
def build_qs(which, start):
    ...
    for k, v in base_filter(which).items():
        p[f"filter[{k}]"] = v
...
def fetch_stream(which, n_batches):
    ...
    if state["total"] is None:
        ...
        filt_wrapped = {f"filter[{k}]": v for k, v in filt.items()}
        r    = http_post(
            config.BASE + "crm.deal.list.json",
            {**filt_wrapped, "start": 0, **{f"select[{i}]": v for i, v in enumerate(SELECT)}},
        )
        if "result" not in r:
