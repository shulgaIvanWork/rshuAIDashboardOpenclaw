  ...
  try {
    ...
    function section(title, ytd, cur, prev, cls, leadsYtd, leadsCur, leadsPrev, qualLeads, mqlCur, mqlPrev, ppYtd, ppLeads, ppQual) {
      ...
      var r = '<div class="kpis"><div class="kpi-header '+cc+'">'+title+'</div>'
        ...
        + '<div class="kpi '+kc+'"><div class="lbl">Оплаченные в&nbsp;периоде</div><div style="display:flex;justify-content:space-between;align-items:baseline"><div class="val-big">'+fmtPct(ytd.paid_created_same_pct)+'%</div>'+(ppYtd?pctDelta(ytd.paid_created_same_pct,ppYtd.paid_created_same_pct):'')+'</div>'+'<div class="sub-sm">'+fmt(ytd.paid_same_period||0)+' из '+fmt(ytd.won_relevant_cnt||0)+' · '+fmtCompact(ytd.paid_same_period_sum||0)+' &#8381;</div>'+pp(fmtPct(ppYtd&&ppYtd.paid_created_same_pct)+'%'+(ppYtd?(' · '+fmtCompact(ppYtd.paid_same_period_sum||0)):''))+'</div>'
