...
```bash
npm install                              # монорепо на npm workspaces (ставит всё сразу)
npm run start --workspace=clover-web     # оболочка + все дашборды на http://127.0.0.1:3000
# данные из Bitrix (вручную/по расписанию, ~20-40 мин):
cd data-service && npm run fetch
```

Нужен `../.env` (см. `.env.example`): `BITRIX_BASE` (вебхук Б24), `SESSION_SECRET`,
опц. `RSHU_YEAR` (по умолчанию текущий год). Node 20+, ESM (`"type":"module"` везде).
